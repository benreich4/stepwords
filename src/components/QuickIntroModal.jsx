    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
      <div className={`w-full max-w-lg rounded-2xl border p-5 shadow-2xl ${lightMode ? 'border-gray-300 bg-white' : 'border-gray-700 bg-gray-900'}`}>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Link to="/quick" onClick={onClose} className="px-3 py-1.5 rounded-md bg-emerald-500 text-white text-sm font-semibold hover:bg-emerald-600 shadow-md shadow-emerald-500/30 transition-all duration-200">Try today's Quick Stepword Puzzle</Link>
        </div>
      </div>
    </div>
  );
